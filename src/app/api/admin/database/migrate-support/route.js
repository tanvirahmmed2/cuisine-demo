...
export async function GET(req) {
  try {
    ...
    try {
      ...
      // Let's check if the table exists first to avoid errors if already renamed
      ...
      // 2. Create new Support Tickets table
      await client.query(`
      ...
      `);
      await client.query(`
        ALTER TABLE restaurant_support_tickets 
      ...
      `);
      await client.query(`
        ALTER TABLE restaurant_support_messages 
        ALTER COLUMN tenant_id SET DEFAULT NULLIF(current_setting('app.current_tenant_id', true), '')::int
      `);
